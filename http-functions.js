  ...
  try {
    ...
    for (const recipient of recipients) {
      ...
      if (bodyParams.length > 0) {
        metaPayload.template.components = metaPayload.template.components || [];
        metaPayload.template.components.push({ type: "body", parameters: bodyParams });
      }
      if (data.couponCode && data.couponButtonIndex !== undefined && data.couponButtonIndex !== null) {
        metaPayload.template.components = metaPayload.template.components || [];
        metaPayload.template.components.push({
          type: "button", sub_type: "copy_code", index: parseInt(data.couponButtonIndex) || 0,
          parameters: [{ type: "coupon_code", coupon_code: String(data.couponCode) }]
        });
      }
